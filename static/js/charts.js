...
function renderSingleMetricChartJS(canvasId, data, xlabel, ylabel, type) {
  ...
  window.chartInstances[canvasId] = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: ylabel,
        data: data.map(d => d.y),
        backgroundColor: type === 'bar' ? '#8a7bfa88' : undefined,
        borderColor: type === 'bar' ? '#8a7bfa' : '#4653f4',
        borderWidth: 2,
        fill: type === 'bar'
      }]
    },
...
function renderDoubleYAxisChartJS(canvasId, dataA, dataB, nameA, nameB, xlabel, typeA, typeB) {
  const ctx = document.getElementById(canvasId).getContext('2d');
  if (window.chartInstances[canvasId]) {
    window.chartInstances[canvasId].destroy();
  }
  window.chartInstances[canvasId] = new Chart(ctx, {
    type: typeA === 'bar' && typeB === 'bar' ? 'bar' : 'line',
    data: {
      ...
      datasets: [
        {
          ...
          type: typeA === 'bar' ? 'bar' : 'line',
          yAxisID: 'y1',
          backgroundColor: typeA === 'bar' ? '#8a7bfa88' : 'transparent',
          borderColor: '#8a7bfa',
          borderWidth: 2,
          fill: false
        },
        {
          ...
          type: typeB === 'bar' ? 'bar' : 'line',
          yAxisID: 'y2',
          backgroundColor: typeB === 'bar' ? '#39a2d688' : 'transparent',
          borderColor: '#39a2d6',
          borderWidth: 2,
          fill: false
        }
    ...
    options: {
      ...
      scales: {
        ...
        y1: {
          type: 'linear',
          position: 'left',
          title: { display: true, text: nameA },
          beginAtZero: true
        },
        y2: {
          type: 'linear',
          position: 'right',
          title: { display: true, text: nameB },
          beginAtZero: true,
